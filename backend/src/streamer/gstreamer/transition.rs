...
pub(super) struct TransitionSchedule {
    ...
    pub completion: Option<gst::ClockTime>,
}
...
pub(super) fn apply_initial(
    ...
    apply(transition, branches, current_duration, true)
}
...
pub(super) fn apply_rolling(
    ...
    apply(transition, branches, current_duration, false)
}
    ...
    match (transition, branches.get(1), current_duration) {
        (TransitionPlan::Cut, Some(next), Some(current_duration)) => {
            ...
            Ok(TransitionSchedule {
                handover: Some(gst::ClockTime::from_nseconds(current_duration.as_nanos() as u64)),
                completion: Some(gst::ClockTime::from_nseconds(current_duration.as_nanos() as u64)),
            })
        }
        (TransitionPlan::NaiveCrossfade { requested_fade }, Some(next), Some(current_duration)) => {
            ...
            Ok(TransitionSchedule {
                handover: Some(gst::ClockTime::from_nseconds(
        ...
        (
            ...
            Ok(TransitionSchedule {
                ...
                completion: Some(gst::ClockTime::from_nseconds(
                    local_fade_start.saturating_add(duration).as_nanos() as u64
                )),
            })
        }
        _ => Ok(TransitionSchedule {
            handover: None,
            completion: None,
        }),
    }
